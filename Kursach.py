...
def genetic_algorithm_schedule(driver_list, shift_duration, num_routes, selected_day, generations=50,
    ...
    for _ in range(population_size):
        ...
        population.append({'schedule': schedule, 'fitness': fitness(schedule)})
    best_schedule = None
    best_fitness = -1
    for gen in range(generations):
        ...
        while len(new_population) < population_size:
            ...
            child1 = {'schedule': child1_schedule, 'fitness': fitness(child1_schedule)}
            child2 = {'schedule': child2_schedule, 'fitness': fitness(child2_schedule)}
        ...
        for individual in new_population:
            if random.random() < mutation_rate:
                ...
                individual['fitness'] = fitness(mutated_schedule)
    ...
    if best_schedule and best_fitness > 0:
        ...
        display_schedule(result_window, df, f"{title_text} ({best_fitness} рейсов):")
    else:
